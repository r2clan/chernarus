...
swSpZadjust = true;						//temp off, needs further investigation
...
_genZadjust = 1;					//High adjustment (in engine units) thats generally added to every spawnpoint
_tmpTstPlace = [5070, 9297, 0];		//Coord's, in [x,y,z] of a preferably flat and unocupied piece of land
//global old
//chBuildhasloot = 50;				//The odds that a building is selected to place loot.
//chSpothasloot = 20;				//The odds that the selected building's spots will have loot(almost like odds per room).
...
[0, 10, 10, 10, 10, 13],	// civil
[1, 10, 10, 10, 10, 13],	// military
[2, 10, 10, 10, 10, 15],	// industrial
[3, 10, 10, 10, 10, 15]		// research
];
			...
			if ((isPlayer _x) && (alive _x)) then {
				...
				//if ((vehicle _x isKindOf "Land") || (vehicle _x isKindOf "Ship")) then {
					...
					if ((count _BaP_list) > 0) then {
						...
						_hndl = [_BaP_list, _spInterval, _chfullfuel, _genZadjust] spawn fn_getBuildingstospawnLoot;
						waitUntil{scriptDone _hndl};
					};
				};
			};
